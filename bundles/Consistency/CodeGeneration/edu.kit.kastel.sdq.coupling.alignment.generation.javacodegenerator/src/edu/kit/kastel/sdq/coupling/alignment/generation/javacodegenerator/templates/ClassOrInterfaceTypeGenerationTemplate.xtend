...
abstract class ClassOrInterfaceTypeGenerationTemplate {
	...
	def String generate() '''
	«IF !Filter.checkFilterList(currentClassOrInterface.name)»
	...
	private def String generateConstructorParameters(
		Class clazz) '''«FOR field : clazz.field SEPARATOR ", "»«field.type.name» «field.name»«ENDFOR»'''
