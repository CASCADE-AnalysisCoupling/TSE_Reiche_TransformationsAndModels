...
public class EDFAJOANAQLAlignmentPS extends AlignmentPS{

	protected static final String USER_SPECIFIC_PATH = "USER_SPECIFIC_PATH";
	protected static final String[] ARG_IDS = { "REPOSITORY_PATH", "SPECIFICATION_MODEL_PATH",
			"DATADICTIONARY_PATH", "JAVA_CODE_BASE_PATH", "CODE_BASE_PACKAGE_NAME", "JAVA_MODEL_PATH",
			"JOANA_MODEL_PATH", "CORRESPONDENCE_MODEL_PATH", "POLICY_STYLE", "EDFA_JOANA_CORRESPONDENCE_PATH"};
	
	
	
	public EDFAJOANAQLAlignmentPS(Registry registry) throws MissingPathIdentifierException {
		super(registry);
	}
	...
	protected String[] getArgsForExecution() {
		...
		List<String> relPaths2 = Arrays.stream(Arrays.copyOfRange(ARG_IDS, 5, 8))
				.map(e -> pathPrefix + super.registry.getFileForID(e).getPath()).collect(Collectors.toList());
		String absPath2 = super.registry.getFileForID(ARG_IDS[8]).getPath();
		String relPath3 = pathPrefix + super.registry.getFileForID("EDFA_JOANA_CORRESPONDENCE_PATH").getPath();
