package edu.kit.kastel.sdq.casestudy.framework.processingsteps.accessanalysiscodeql;
...
import edu.kit.kastel.sdq.casestudy.framework.processingsteps.accessanalysiscodeql.adapters.AccessAnalysis2CodeQLAdapter;

public class CodeQLAlignmentPS extends AlignmentPS {

	protected static final String USER_SPECIFIC_PATH = "USER_SPECIFIC_PATH";
	protected static final String[] ARG_IDS = { "REPOSITORY_PATH", "CONFIDENTIALITY_MODEL_PATH", "ACCESS_ANALYSIS_CONFIGURATIONS_PATH", 
			"CODE_BASE_PACKAGE_NAME", "JAVA_MODEL_PATH", "CODEQL_MODEL_PATH", "CODEQL_CONFIGURATIONS_PATH", "PCM_JAVA_CORRESPONDENCE_MODEL_PATH",
			"CODEQL_FOLDER_PATH", "ACCESSANALYSIS_CODEQL_CORRESPONDENCE_PATH", "POLICY_STYLE" };
	...
	protected String[] getArgsForExecution() {
		...
		List<String> relPaths2 = Arrays.stream(Arrays.copyOfRange(ARG_IDS, 4, 10))
				.map(e -> pathPrefix + super.registry.getFileForID(e).getPath()).collect(Collectors.toList());
