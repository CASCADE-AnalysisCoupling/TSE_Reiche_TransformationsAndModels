...
public class EDFACodeQLAlignmentPS extends AlignmentPS{
	...
	protected static final String[] ARG_IDS = { "REPOSITORY_PATH", "SPECIFICATION_MODEL_PATH",
			"DATADICTIONARY_PATH", "CODE_BASE_PACKAGE_NAME", "JAVA_MODEL_PATH", "CODEQL_MODEL_PATH",
			"CORRESPONDENCE_MODEL_PATH", "CODEQL_FOLDER_PATH", "POLICY_STYLE", "EDFA_CODEQL_CORRESPONDENCE_PATH"};
	...
	protected String[] getArgsForExecution() {
		...
		List<String> relPaths2 = Arrays.stream(Arrays.copyOfRange(ARG_IDS, 4, 8))
				.map(e -> pathPrefix + super.registry.getFileForID(e).getPath()).collect(Collectors.toList());
		String absPath2 = super.registry.getFileForID(ARG_IDS[8]).getPath();
		String relPath3 = pathPrefix + super.registry.getFileForID("EDFA_CODEQL_CORRESPONDENCE_PATH").getPath();
